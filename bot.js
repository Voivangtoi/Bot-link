...
const MIN_WITHDRAW_VN = 100; 
const USDT_RATE = 25000; 
const MIN_WITHDRAW_USDT_VND = 100; 

const COOLDOWN_TIME = 10 * 60 * 1000; // 10 phút chờ
...
// Tạo ID 10 số ngẫu nhiên cố định cho tài khoản
...
function getUser(data, userId) {
  if (!data.users[userId]) {
    data.users[userId] = {
      userId,
      accountCode: generateTenDigitId(), // ID 10 số cố định
      balance: 0,
      lastBypassTime: 0,
    ...
    // Đảm bảo user cũ cũng có ID 10 số
