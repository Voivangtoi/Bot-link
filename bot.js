...
function handleUpdate(update) {
    ...
    if (text.startsWith('/start')) {
      ...
      if (parts[1] && !user.referredBy && Number(parts[1]) !== userId) {
        ...
        if (db.users[refId]) {
          ...
          if (ref) ref.refCount = (ref.refCount || 0) + 1; // Tăng đếm ref khi có người bấm link start
        }
      }
      saveData(db);
  ...
  if (update.callback_query) {
    ...
    if (dataKey === 'ADMIN_PANEL') {
      ...
      const refLink = `https://t.me/vuotlinkbot?start=${userId}`;
      sendTelegram('sendMessage', {
        chat_id: chatId,
...
mpWithdraw = { type: 'ATM', bankName: parts[0].toUpperCase(), accountNo: parts[1], accountName: parts[2].toUpperCase() };
      user.withdrawStep = 'WAITING_AMOUNT';
      saveData(db);
      sendTelegram('sendMessage', { chat_id: chatId, text: `💰 Nhập số tiền muốn rút (Từ ${MIN_WITHDRAW_VN.toLocaleString()} VNĐ đến ${MAX_WITHDRAW_VN.toLocaleString()} VNĐ):` });
      return;
